...
class ServiceUser extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('company_name, city,zipcode', 'required'),
			array('zipcode, jobsdone', 'numerical', 'integerOnly'=>true),
			array('email, password, city', 'length', 'max'=>45),
			array('company_name, company_logo', 'length', 'max'=>100),
			array('registered, lastlogin', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, email, password, company_name, company_logo, city, zipcode, jobsdone, registered, lastlogin', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			...
			'newAttributes' => array(self::HAS_MANY, 'NewAttribute', 'service_id'),
