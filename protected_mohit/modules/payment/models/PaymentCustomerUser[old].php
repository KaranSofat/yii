...
class PaymentCustomerUser extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('email,cname,caddress,clname,zipcode, phone, city', 'required'),
			array('phone', 'numerical', 'integerOnly'=>true),
			array('email, password, cname, clname, caddress, type', 'length', 'max'=>45),
			
            array('email','email'),
            array('email', 'uniqueEmail'),

			array('zipcode, city', 'length', 'max'=>100),
			array('cregistered, clastlogin', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, email, password, cname, clname, zipcode, phone, city, caddress, cregistered, clastlogin, type', 'safe', 'on'=>'search'),
		);
	}
