...
class MsgDetails extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('conversation_id, user_type, user_id, to, msg, date', 'required'),
			array('conversation_id, user_id, to', 'numerical', 'integerOnly'=>true),
			array('user_type', 'length', 'max'=>100),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, conversation_id, user_type, user_id, to, msg, date', 'safe', 'on'=>'search'),
		);
	}
