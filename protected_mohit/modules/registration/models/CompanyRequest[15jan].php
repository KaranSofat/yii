...
class CompanyRequest extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('service_id, booking_id, cleaningtime, postcode, cleaningdate, date', 'required'),
			array('service_id, booking_id, job_status', 'numerical', 'integerOnly'=>true),
			array('cleaningtime, postcode', 'length', 'max'=>100),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, service_id, booking_id, job_status, cleaningtime, postcode, cleaningdate, date', 'safe', 'on'=>'search'),
		);
	}
